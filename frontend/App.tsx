...
// Fix for error on line 75: 'Classification' cannot be used as a value because it was imported using 'import type'.
...
const App: React.FC = () => {
    ...
    if (isLoading) {
      ...
      intervalId = window.setInterval(() => {
        ...
        timeoutId = window.setTimeout(() => {
          ...
          setMessageOpacity(1); // Start fade in
        }, 300); // Matches transition duration

      }, 1500); // Total cycle time per message
    }
  ...
  const handleFileParsed = useCallback((data: Partial<ExoplanetData>) => {
    // Validate that all keys in data exist in initialData and values are numbers
  ...
  const handleBatchUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Auto-detect: if only 1 result, show single mode, otherwise batch mode
  ...
  const classifyExoplanet = async (data: ExoplanetData): Promise<ClassificationResponse> => {
    ...
    // Optional: Validate the response from the backend
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If data hasn't been changed, show the custom "Earth" message
    if (JSON.stringify(formData) === JSON.stringify(initialData)) {
      setTimeout(() => {
        setResult({
          classification: Classification.FALSE_POSITIVE,
          rationale: "Hold up. Our systems are showing this star is a 'Sun' and this planet is a... wait for it... 'Earth.' This isn't a new discovery, this is your home address. Please reset and try again, we're looking for exotic exoplanets, not the one outside your window. 🤦‍♂️"
        });
        setIsLoading(false);
  ...
  return (
    <div className="min-h-screen text-slate-300 flex flex-col items-center p-4 pt-6 font-sans relative overflow-hidden">
      ...
      <main className="w-full max-w-6xl mx-auto flex-grow mb-10 z-10">
          ...
          <div className="p-6 md:p-8">
            ...
            <form onSubmit={handleSubmit}>
              <BentoCardGrid gridRef={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                ...
                {/* Combined card for celestial coordinates */}
                <ParticleCard
                  key="coordinates"
                  disableAnimations={shouldDisableAnimations}
        ...
        {/* Batch Classification Results (Multiple Planets) */}
        {batchResults && (
